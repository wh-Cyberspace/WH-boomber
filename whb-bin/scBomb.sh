#!/bin/bash
LPARTH=`pwd`
homemenu="whmenu.sh"
RED=$'\e[1;31m'
GREEN=$'\e[1;32m'
YELLOW=$'\e[1;33m'
BLUE=$'\e[1;34m'
RESTORE=$'\e[0m'
clear
echo "${YELLOW}"
echo " "
echo "WWWWWWWW                           WWWWWWWWHHHHHHHHH     HHHHHHHHH      "               
echo "W::::::W                           W::::::WH:::::::H     H:::::::H       "              
echo "W::::::W                           W::::::WH:::::::H     H:::::::H       "              
echo "W::::::W                           W::::::WHH::::::H     H::::::HH                     "
echo " W:::::W           WWWWW           W:::::W   H:::::H     H:::::H                       "
echo "  W:::::W         W:::::W         W:::::W    H:::::H     H:::::H                       "
echo "   W:::::W       W:::::::W       W:::::W     H::::::HHHHH::::::H                       "
echo "    W:::::W     W:::::::::W     W:::::W      H:::::::::::::::::H        ---------------"
echo "     W:::::W   W:::::W:::::W   W:::::W       H:::::::::::::::::H        -:::::::::::::-"
echo "      W:::::W W:::::W W:::::W W:::::W        H::::::HHHHH::::::H        ---------------"
echo "       W:::::W:::::W   W:::::W:::::W         H:::::H     H:::::H                       "
echo "        W:::::::::W     W:::::::::W          H:::::H     H:::::H                       "
echo "         W:::::::W       W:::::::W         HH::::::H     H::::::HH                     "
echo "          W:::::W         W:::::W          H:::::::H     H:::::::H                     "
echo "           W:::W           W:::W           H:::::::H     H:::::::H                     "
echo "            WWW             WWW            HHHHHHHHH     HHHHHHHHH                     "
echo "${BLUE}"
echo ""
echo " @@@@@@   @@@@@@@@@@    @@@@@@                     @@@@@@@   @@@@@@   @@@       @@@     "
echo "@@@@@@@   @@@@@@@@@@@  @@@@@@@                    @@@@@@@@  @@@@@@@@  @@@       @@@     "
echo "!@@       @@! @@! @@!  !@@             @@!        !@@       @@!  @@@  @@!       @@!     "
echo "!@!       !@! !@! !@!  !@!             !@!        !@!       !@!  @!@  !@!       !@!     "
echo "!!@@!!    @!! !!@ @!@  !!@@!!       @!@!@!@!@     !@!       @!@!@!@!  @!!       @!!     "
echo " !!@!!!   !@!   ! !@!   !!@!!!      !!!@!@!!!     !!!       !!!@!!!!  !!!       !!!     "
echo "     !:!  !!:     !!:       !:!        !!:        :!!       !!:  !!!  !!:       !!:     "
echo "    !:!   :!:     :!:      !:!         :!:        :!:       :!:  !:!   :!:       :!:    "
echo ":::: ::   :::     ::   :::: ::                     ::: :::  ::   :::   :: ::::   :: :::: "
echo ":: : :     :      :    :: : :                      :: :: :   :   : :  : :: : :  : :: : : "
echo ""
echo "${RED}"
echo " ______      ____       ____       __    __     ______     _____   ______   "
echo "(_   _ \    / __ \     / __ \      \ \  / /    (_   _ \   / ___/  (   __ \  "
echo "  ) (_) )  / /  \ \   / /  \ \     () \/ ()      ) (_) ) ( (__     ) (__) ) "
echo "  \   _/  ( ()  () ) ( ()  () )    / _  _ \      \   _/   ) __)   (    __/  "
echo "  /  _ \  ( ()  () ) ( ()  () )   / / \/ \ \     /  _ \  ( (       ) \ \  _ "
echo " _) (_) )  \ \__/ /   \ \__/ /   /_/      \_\   _) (_) )  \ \___  ( ( \ \_))"
echo "(______/    \____/     \____/   (/          \) (______/    \____\  )_) \__/ "

echo "${GREEN}"

if [[ -s update.speedx ]];then
echo "All Requirements Found...."
else

echo Press Enter To Continue...
read ff
fi
while :
do
rm *.xxx >/dev/null 2>&1
clear
LPARTH=`pwd`
RED=$'\e[1;31m'
GREEN=$'\e[1;32m'
YELLOW=$'\e[1;33m'
BLUE=$'\e[1;34m'
RESTORE=$'\e[0m'
homepage=$LPARTH/WHB.sh
homemenu="whmenu.sh"

echo " █     █░ ██░ ██ "
echo "▓█░ █ ░█░▓██░ ██▒"
echo "▒█░ █ ░█ ▒██▀▀██░"
echo "░█░ █ ░█ ░▓█ ░██ "
echo "░░██▒██▓ ░▓█▒░██▓"
echo "░ ▓░▒ ▒   ▒ ░░▒░▒"
echo "  ▒ ░ ░   ▒ ░▒░ ░"
echo "  ░   ░   ░  ░░ ░"
echo "    ░     ░  ░  ░"
echo "                 "


echo "                                                                                                          "     
			echo " ██╗  ██╗ █████╗  ██████╗██╗  ██╗███████╗██████╗     ███████╗██╗  ██╗██████╗ ██╗      ██████╗ ██╗████████╗"
			echo " ██║  ██║██╔══██╗██╔════╝██║ ██╔╝██╔════╝██╔══██╗    ██╔════╝╚██╗██╔╝██╔══██╗██║     ██╔═══██╗██║╚══██╔══╝"
			echo " ███████║███████║██║     █████╔╝ █████╗  ██████╔╝    █████╗   ╚███╔╝ ██████╔╝██║     ██║   ██║██║   ██║   "
			echo " ██╔══██║██╔══██║██║     ██╔═██╗ ██╔══╝  ██╔══██╗    ██╔══╝   ██╔██╗ ██╔═══╝ ██║     ██║   ██║██║   ██║   "
			echo " ██║  ██║██║  ██║╚██████╗██║  ██╗███████╗██║  ██║    ███████╗██╔╝ ██╗██║     ███████╗╚██████╔╝██║   ██║   "
			echo " ╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝    ╚══════╝╚═╝  ╚═╝╚═╝     ╚══════╝ ╚═════╝ ╚═╝   ╚═╝   "
			echo "                                                                                                          "
echo "${RESTORE}"
echo "${RED}=============================================================================================================="
echo "${RESTORE}"
echo "${BLUE}"
echo "LEGAL DISCLAIMER >>"
echo "${RED}"
echo "Usage of WH-HackerExploit Hacking Tool for attacking targets without prior mutual "
echo "consent is illegal. It's the end user's responsibility to obey all applicable local," 
echo "state and federal laws. Developers assume no liability and are not responsible"
echo "for any misuse or damage caused by this program. Only use for educational purposes."
echo ""
echo "${RED}=============================================================================================================="
echo "${RESTORE}"
 echo "${BLUE}"
echo -e "\e[1;31m"
echo "${GREEN}"
echo " "
echo "Press 1 To  Start SMS Bomber "
echo "Press 2 To  Start Call Bomber "
echo "Press 3 To  Update (Works On Linux And Linux Emulators) "
echo "Press 4 To  View Features "
echo "Press 5 To  Back "
echo ""
read -p ' WH :~ ' ch
if [ $ch -eq 1 ];then
clear
echo -e "\e[1;32m"
rm *.xxx >/dev/null 2>&1
python3 $LPARTH/whb-bin/bomber.py
rm *.xxx >/dev/null 2>&1
exit 0
elif [ $ch -eq 2 ];then
clear
echo -e "\e[1;32m"
echo 'Call Bomb By SpeedX'> call.xxx
python3 $LPARTH/whb-bin/bomber.py call
rm *.xxx >/dev/null 2>&1
exit 0
elif [ $ch -eq 3 ];then
clear
apt install git -y
echo -e "\e[1;34m Downloading Latest Files..."
git clone https://github.com/TheSpeedX/WH-Boom
if [[ -s WH-Boom/WH-Boom.sh ]];then
cd WH-Boom
cp -r -f * .. > temp
cd ..
rm -rf  WH-Boom >> temp
rm update.speedx >> temp
rm temp
chmod +x WH-Boom.sh
fi
echo -e "\e[1;32m WH-Boom Will Restart Now..."
echo -e "\e[1;32m All The Required Packages Will Be Installed..."
echo -e "\e[1;34m Press Enter To Proceed To Restart..."
read a6
./WH-Boom.sh
exit
elif [ $ch -eq 4 ];then
clear
echo -e "\e[1;33m"
figlet WH-Boom
echo -e "\e[1;34mCreated By \e[1;34m"


echo "██╗    ██╗██╗  ██╗      ██╗  ██╗ █████╗  ██████╗██╗  ██╗███████╗██████╗ ███████╗██╗  ██╗██████╗ ██╗      ██████╗ ██╗████████╗"
echo "██║    ██║██║  ██║      ██║  ██║██╔══██╗██╔════╝██║ ██╔╝██╔════╝██╔══██╗██╔════╝╚██╗██╔╝██╔══██╗██║     ██╔═══██╗██║╚══██╔══╝"
echo "██║ █╗ ██║███████║█████╗███████║███████║██║     █████╔╝ █████╗  ██████╔╝█████╗   ╚███╔╝ ██████╔╝██║     ██║   ██║██║   ██║   "
echo "██║███╗██║██╔══██║╚════╝██╔══██║██╔══██║██║     ██╔═██╗ ██╔══╝  ██╔══██╗██╔══╝   ██╔██╗ ██╔═══╝ ██║     ██║   ██║██║   ██║   "
echo "╚███╔███╔╝██║  ██║      ██║  ██║██║  ██║╚██████╗██║  ██╗███████╗██║  ██║███████╗██╔╝ ██╗██║     ███████╗╚██████╔╝██║   ██║   "
echo " ╚══╝╚══╝ ╚═╝  ╚═╝      ╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝╚═╝     ╚══════╝ ╚═════╝ ╚═╝   ╚═╝   "
echo "                                                                                                                             "





echo  " "
echo -e "\e[1;32m                   Features\e[1;34m"
echo "  [+] Unlimited And Super-Fast Bombing"
echo "  [+] International Bombing"
echo "  [+] Call Bombing "
echo "  [+] Protection List"
echo "  [+] Automated Future Updates"
echo "  [+] Easy To Use And Embed in Code"
echo -e "\e[1;32m                   Contributors\e[1;33m"
#echo -e "\e[1;33m      [*]  SpeedX   \e[1;31m"
#echo "         [-] Mail At: ggspeedx29@gmail.com"
echo -e "\e[1;33m      [*]  The Black Hacker Roxstar   \e[1;31m"
#echo "         [-] Ping At: http://wa.me/917600140353"
echo -e "\e[1;33m      [*]  Rieltar   \e[1;31m"
#echo "         [-] Ping At: https://t.me/Rieltar"
echo -e "\e[1;33m      [*]  0n1cOn3 (Stefan)   \e[1;31m"
#echo "         [-] Mail At: 0n1cOn3@gmx.ch"
echo ""
echo ""
echo -e "\e[1;31m This Script is Only For Educational Purposes or To Prank.\e[0m"
echo -e "\e[1;31m Do not Use This To Harm Others. \e[0m"
echo -e "\e[1;31m I Am Not Responsible For The Misuse Of The Script. \e[0m"
echo -e "\e[1;32m Make Sure To Update it If It Does not Work.\e[0m"
echo  " "

echo -e "\e[4;31m That's All !!!\e[0m"
echo -e "\e[1;34m For Any Queries contuct github Me!!!\e[0m"

echo -e "\e[1;32m       Github: https://github.com/wh-hackerexploit \e[0m"
echo -e "\e[4;32m   YouTube Page: https://www.youtube.com/channel/UCj6ekUzjItnjP6T7I9r1WMA?sub_confirmation=1 \e[0m"
echo "Press Enter To Go Home"
read a3
clear
elif [ $ch -eq 5 ];then
clear
echo -e "\e[1;31m"
figlet WH-Boom
echo -e "\e[1;34m Created By \e[1;32m"
			echo " ██╗  ██╗ █████╗  ██████╗██╗  ██╗███████╗██████╗     ███████╗██╗  ██╗██████╗ ██╗      ██████╗ ██╗████████╗"
			echo " ██║  ██║██╔══██╗██╔════╝██║ ██╔╝██╔════╝██╔══██╗    ██╔════╝╚██╗██╔╝██╔══██╗██║     ██╔═══██╗██║╚══██╔══╝"
			echo " ███████║███████║██║     █████╔╝ █████╗  ██████╔╝    █████╗   ╚███╔╝ ██████╔╝██║     ██║   ██║██║   ██║   "
			echo " ██╔══██║██╔══██║██║     ██╔═██╗ ██╔══╝  ██╔══██╗    ██╔══╝   ██╔██╗ ██╔═══╝ ██║     ██║   ██║██║   ██║   "
			echo " ██║  ██║██║  ██║╚██████╗██║  ██╗███████╗██║  ██║    ███████╗██╔╝ ██╗██║     ███████╗╚██████╔╝██║   ██║   "
			echo " ╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝    ╚══════╝╚═╝  ╚═╝╚═╝     ╚══════╝ ╚═════╝ ╚═╝   ╚═╝   "
			echo "                                                                                                          "

echo -e "\e[1;34m For Any Queries contuct Me on github!!!\e[0m"

echo -e "\e[1;32m       Github: https://github.com/wh-hackerexploit \e[0m"
echo -e "\e[4;32m   YouTube Page: https://www.youtube.com/channel/UCj6ekUzjItnjP6T7I9r1WMA?sub_confirmation=1 \e[0m"
echo " "
exit 0
else
echo -e "\e[4;32m Invalid Input !!! \e[0m"
echo "Press Enter To Go Home"
read a3
clear
fi
done

